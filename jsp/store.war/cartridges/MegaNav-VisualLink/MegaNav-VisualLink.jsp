<dsp:page>
	...
	<c:choose>
		<c:when test="${not empty contentItem.videoUrl}">
			...
			<!-- ${contentItem.isModal} -->
			<li class="video-link" data-module="cbox" data-transition="elastic"
				data-speed="500" data-class="video-modal" data-width="680"
				data-title="false">
					<a href="${modalUrl}" class="video cboxElement image-link" aria-label="${contentItem.title}" tabindex="0"><span class="icon-play"></span>
						<img src="${contentItem.videoImageUrl}" alt="${contentItem.imageAltText}" />
					</a>
				 <a	href="${modalUrl}" class="video cboxElement" aria-label="${contentItem.title}" tabindex="0">${contentItem.title}</a>
