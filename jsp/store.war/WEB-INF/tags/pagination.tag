...
<%@ attribute name="nextPageLinkRenderer" fragment="true" %>
<%@ attribute name="prevPageLinkRenderer" fragment="true" %>
<%@ attribute name="viewAllLinkRenderer" fragment="true" %>

<%@ variable name-given="nextValue" variable-class="java.lang.Integer" %>
<%@ variable name-given="nextStart" variable-class="java.lang.String" %>
<%@ variable name-given="prevValue" variable-class="java.lang.Integer" %>
<%@ variable name-given="prevStart" variable-class="java.lang.String" %>
    ...
    <ul id="atg_store_pagination">
      ...
      <c:if test="${size % arraySplitSize == 0}">    
        <dsp:getvalueof var="p_finalPageSize" value="${size/arraySplitSize}" />
		  ...
		  <%-- prevPageLinkRenderer --%>
	  
	 	<c:if test="${start > arraySplitSize}">
			<dsp:getvalueof var="prevValue" value="${start - arraySplitSize}" vartype="java.lang.Integer" />
			<c:set var="prevStart" value="${prevValue}"/>
			<jsp:invoke fragment="prevPageLinkRenderer"/>
		</c:if>
	  ...
	  <%-- nextPageLinkRenderer --%>
	  
	  <c:choose>
		<c:when test="${empty start}">
			<dsp:getvalueof var="nextValue" value="${arraySplitSize + 1}" vartype="java.lang.Integer" />
		</c:when>
		<c:otherwise>
			<dsp:getvalueof var="nextValue" value="${arraySplitSize + start}" vartype="java.lang.Integer" />
		</c:otherwise>
	  </c:choose>
	  <c:choose>
		<c:when test="${nextValue < size}">
			<c:set var="nextStart" value="${nextValue}"/>
			<jsp:invoke fragment="nextPageLinkRenderer"/>
		</c:when>
		<c:otherwise>
			<c:set var="nextStart" value="-1"/>
		</c:otherwise>
	  </c:choose>
