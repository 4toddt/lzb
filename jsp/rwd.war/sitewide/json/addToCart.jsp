<%--
    - File Name: addToCart.jsp
    - Author(s): DMI UX Team
    - Copyright Notice:
    - Description: This page returns a json data response for adding a product to the cart.
    - Required Parameters:
    - Optional Parameters:
--%>
				...
				<dsp:droplet name="LZBCurrentItemAddedToCartDroplet">
					...
					<dsp:oparam name="output">
						...
						<json:object name="addedItem">
							...
							<dsp:include page="/checkout/includes/styleCollectionsByItem.jsp">
							...
							<c:set var="marxentImageURL" value="${contentHostName}${imagePath}${cdpImage}"/>
							...
							<dsp:droplet name="LZBPDPURLDroplet">
								...
								<dsp:oparam name="output">
									...
									<json:property name="prodpdpurl">${contextPath}${prodpdpurl}</json:property>
									<json:property name="editLink">${contextPath}${prodpdpurl}?ac=edit&commid=${commerceItemId}</json:property>
								</dsp:oparam>
							</dsp:droplet>
							...
							<json:property name="rating"></json:property>
							...
							<json:property name="protectionPlanSmallImgURL">
								${contentHostName}<dsp:valueof param="addedItemInfo.protectionPlanSku.smallImage.url" valueishtml="true"/>
							</json:property>
							<json:property name="protectionPlanLargrImgURL">
								${contentHostName}<dsp:valueof param="addedItemInfo.protectionPlanSku.largeImage.url" valueishtml="true"/>
							</json:property>
											...
											<json:object>
												<dsp:droplet name="LZBPDPURLDroplet">
													...
													<dsp:oparam name="output">
														...
														<json:property name="url">${contextPath}${recordURL}</json:property>
													</dsp:oparam>
												</dsp:droplet>
											<json:property name="name"><dsp:valueof value="${element.attributes['sku.displayName']}" valueishtml="true" /></json:property>
											<json:property name="imageUrl"><dsp:valueof value="${contentHostName}${storeConfig.imagePath}${element.attributes['sku.cdpDefaultImage']}" valueishtml="true" /></json:property>
											</json:object>
										</c:forEach>
									</dsp:oparam>
								</dsp:droplet>
