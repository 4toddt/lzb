<%--
  - File Name: displayPreferredStoreHeader.jsp
  - Author(s): DMI UX Team
  - Copyright Notice:
  - Description: This page lays out the Desktop Header Store Details Flyout Navigation in the Utility Navigation Bar.
  - Required Parameters:
  - Optional Parameters:
--%>
<dsp:page>
	...
	<c:when test="${ store != null }">
		...
		<a href="${contextLinkPath}/storeLocator/storeLocator.jsp" class="utility store sub-nav-button" tabindex="0">
			<c:set var="storeName" value ="${store.name}"/>
			<c:set var="storeNameNew" value ="${fn:replace(storeName, 'é', '&#233;')}"/>
			<div class="store-name" data-store-id="${storeId}"><dsp:valueof value="${storeNameNew}" valueishtml="true"/></div>
			<div class="store-location">${store.city}, ${store.stateAddress}</div>
			<span class="icon-arrow-down"></span>
		</a>
		<div class="store-menu sub-nav-menu">
			<div class="store-content">
				<div class="sub-nav clearfix">
					...
					<c:if test="${not empty storeURL}">
						...
						<a href="${contextLinkPath}${storeURL}" id="visit-preffered-store" class="left" aria-label="${visitYourStore}" tabindex="0">${visitYourStore}</a>
					</c:if>
					<fmt:message key="preferdStore.moreStore" var="moreStore"/>
					<a href="${contextLinkPath}/storeLocator/storeLocator.jsp" class="right" aria-label="${moreStore}" tabindex="0">${moreStore}</a>
				</div>
				<div class="map">
				...
				<div class="store-details">
					...
					<a href="https://www.google.com/maps/dir/Current+Location/${store.address1}${store.address2}${store.address3}+${store.city}+${store.stateAddress}+${store.postalCode}" target="_blank" class="button secondary medium" aria-label="${getDirection}" tabindex="0">${getDirection}</a>
				</div>
			</div>

			<c:if test="${not empty storePromo}">
				<%-- Display Store Dropdown Promo --%>
				<dsp:renderContentItem contentItem="${storePromo}" />
			</c:if>

		</div>
	</c:when>
	<c:otherwise>
	<a href="/storeLocator/storeLocator.jsp" class="utility store default" tabindex="0">Store Locator</a>
	</c:otherwise>
	</c:choose>
</dsp:page>