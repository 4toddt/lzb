<dsp:page>
	...
	<fmt:message key="preferdStore.getDirection" var="getDirection"/>
		...
		<footer role="contentinfo" class="large-up-legacy">
			<%--
			...
			<div class="footer-promos">
				<%-- START Store Content --%>
				<div class="half-page-promo footer-store-info clearfix" data-store-id="${storeId}" tabindex="0">
					<c:choose>
							...
							<div class="content">
								<div class="promo-col col-1">
									...
									<c:if test="${not empty storeURL}">
										<a class="store-details" href="${contextLinkPath}${storeURL}" aria-label="${storeDetails}" tabindex="0">${storeDetails}</a>
									</c:if>
									<a class="change-store" href="${contextLinkPath}/storeLocator/storeLocator.jsp" aria-label="${moreStore}" tabindex="0">${moreStore}</a>
									<a href="https://www.google.com/maps/dir/Current+Location/${store.address1}${store.address2}${store.address3}+${store.city}+${store.stateAddress}+${store.postalCode}" target="_blank" class="button secondary small store-direction" aria-label="${getDirection}" tabindex="0">${getDirection}</a>
								</div>
						...
						<c:otherwise>
							...
							<div class="content">
								<div class="promo-col col-1">
									...
									<a href="${contextLinkPath}${actionURLink}" class="button secondary small" aria-label="${content.CTAText}" tabindex="0">${content.CTAText}</a>
								</div>
								<div class="promo-col col-2">
									<img src="${content.storeImageUrl}" alt="${content.storeImageAlt}" />
								</div>
			...
			<%-- END Promos --%>
				...
				<div class="email-signup">
					<dsp:include src="${contextPath}/account/SignUpEmail.jsp" />
				</div><%-- .utilities --%>
				<div class="footer-links">
					<ul>

						<dsp:include src="${contextPath}/global/includes/customerCare.jsp"/>
						...
						<dsp:include src="${contextPath}/global/includes/partnerSites.jsp"/>
				...
				<div class="mobile-footer-links">
					<ul>
					<c:forEach items="${content.mobileLinks.subLinks}" var="mobileFooterLink">
						<dsp:renderContentItem contentItem="${mobileFooterLink}" />
					</c:forEach>
					</ul>
				</div>
