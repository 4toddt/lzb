...
(function (global, $, namespace){
	...
	var messages = global[namespace].constants.messages,
			...
			rules = {
				email : {
					...
					msg : function () {
						return messages.prefix + ' ' + messages.email;
					}
				},
				uspostal : {
					check: function (value, field) {
					...
					msg : function () {
						return messages.prefix + ' ' + messages.uspostal;
					}
				},
				capostal : {
					check: function (value, field) {
						if (value) {
							return testPattern (value, "^[ABCEGHJKLMNPRSTVXY]\\d[ABCEGHJKLMNPRSTVWXYZ]\\s?\\d[ABCEGHJKLMNPRSTVWXYZ]\\d$");
						}
						return true;
					},
					msg : function () {
						return messages.prefix + ' ' + messages.capostal;
					}
				},
				postal : {
					check: function(value, field){
				...
				usorcapostal : {
					check: function (value, field) {
						if (value) {
							...
							var catest = testPattern (upperCaseValue, "^[ABCEGHJKLMNPRSTVXY]\\d[ABCEGHJKLMNPRSTVWXYZ]\\s?\\d[ABCEGHJKLMNPRSTVWXYZ]\\d$");
					...
					msg : function () {
						return messages.prefix + ' ' + messages.usorcapostal;
					}
				},
				phone : {
					check: function(value, field){
				...
				usphone : {
					check: function(value, field) {
						if (value) {
							...
							return testPattern (value3, "^([0-9]{3}) ([0-9]{3})\-([0-9]{4})$");
						}
						return true;
					},
					msg : function () {
						return messages.prefix + ' ' + messages.usphone;
					}
				},
				...
				internationalphone : {
					...
					msg : function () {
						return messages.prefix + ' ' + messages.internationalphone;
					}
				},
				...
				numeric : {
					...
					msg : function (field) {
						...
						if (fieldName !== undefined && fieldName !== '') {
							return messages.prefix + ' ' + fieldName + ' ' + messages.numeric.named;
						}
						return messages.prefix + ' ' + messages.numeric.unnamed;
					}
				},
				alphanumeric : {
					check: function(value, field) {
					...
					msg : function (field) {
						var fieldName = field.attr('data-fieldname');
						if (fieldName !== undefined && fieldName !== '') {
							return messages.prefix + ' ' + fieldName + ' ' + messages.alphanumeric.named;
						}
						return messages.prefix + ' ' + messages.alphanumeric.unnamed;
					}
				},
				creditcard : {
					...
					msg : function () {
						return messages.prefix + ' ' + messages.creditcard;
					}
				},
				creditcardExpiration : {
					check: function(value, field) {
						var month = value.substring(0,2).replace(/^0+(?!\.|$)/, ''),
					...
					msg : function () {
						return messages.prefix + ' ' + messages.creditcardExpiration;
					}
				},
				emailOrPhoneRequired : {
					check: function(value, field) {
						var $emailField = $('.optionalMandatory #email'),
							$phoneField = $('.optionalMandatory #phoneNumber'),
							emailValue = $emailField.val(),
							phoneValue = $phoneField.val();
						
						if ($.trim(emailValue) !== '' || $.trim(phoneValue) !== '') {
							this.showFieldSuccess($emailField);
							this.showFieldSuccess($phoneField);
							
							if ($.trim(phoneValue) == '') {
								$('.optionalMandatory label[for="phoneNumber"] .required-label').hide();
							} else if ($.trim(emailValue) == '') {
								$('.optionalMandatory label[for="email"] .required-label').hide();
							}
							
							return true;
						} else if ($.trim(emailValue) == '' && $.trim(phoneValue) == '') {
							$('.optionalMandatory label[for="email"] .required-label').show();
							$('.optionalMandatory label[for="phoneNumber"] .required-label').show();
							return false;
						}
					},
					msg : function (field) {
						var fieldName = field.attr('data-fieldname');
						if (fieldName !== undefined && fieldName !== '') {
							return messages.prefix + ' ' + fieldName + ' ' + messages.required.named;
						}
						return messages.prefix + ' ' + messages.required.unnamed;
					}
				},
				required : {
					check: function(value, field) {
						...
						else if (field.hasClass('radio-group')) {
							var radioName = field.find('input[type="radio"]:eq(0)').attr('name');
				...
				minlength : {
					...
					msg : function (field) {
						var fieldName = field.attr('data-fieldname');
						if (fieldName !== undefined && fieldName !== '') {
							return messages.prefix + ' ' + fieldName + ' ' + messages.minlength.named;
						}
						return messages.prefix + ' ' + messages.minlength.unnamed;
					}
				},
				maxlength : {
					check : function (value, field) {
					...
					msg : function (field) {
						var fieldName = field.attr('data-fieldname');
						if (fieldName !== undefined && fieldName !== '') {
							return messages.prefix + ' ' + fieldName + ' ' + messages.maxlength.named;
						}
						return messages.prefix + ' ' + messages.maxlength.unnamed;
					}
				},
				matchPassword : {
					check : function (value, field) {
					...
					msg : function () {
						return messages.prefix + ' ' + messages.matchPassword;
					}
				},
				matchEmail : {
					check : function (value, field) {
					...
					msg : function () {
						return messages.prefix + ' ' + messages.matchEmail;
					}
				},
				qty : {
					check: function(value, field) {
					...
					msg : function () {
						return messages.prefix + ' ' + messages.qty;
					}
				},
				password : {
					check: function(value, field) {
					...
					msg : function () {
						return messages.prefix + ' ' + messages.password;
					}
				},
				validDateOfBirth : {
					check: function(value, field) {
					...
					msg : function () {
						return messages.prefix + ' ' + messages.dateOfBirthDate;
					}
				},
				validDateOfBirthAge : {
					check: function(value, field) {
					...
					msg : function () {
						return messages.prefix + ' ' + messages.dateOfBirthAge;
					}
				},
				giftMessage : {
					check: function(value, field) {
					...
					msg: function() {
						return messages.prefix + ' ' + messages.invalidGiftMessage;
					}
				},
				nameField : {
					check: function(value, field) {
					...
					msg : function (field) {
						var fieldName = field.attr('data-fieldname');
						if (fieldName !== undefined && fieldName !== '') {
							return messages.prefix + ' ' + fieldName + ' ' + messages.nameField.named;
						}
						return messages.prefix + ' ' + messages.nameField.unnamed;
					}
				},
				name : {
					check: function(value, field) {
					...
					msg : function (field) {
						var fieldName = field.attr('data-fieldname');
						if (fieldName !== undefined && fieldName !== '') {
							return messages.prefix + ' ' + fieldName + ' ' + messages.name.named;
						}
						return messages.prefix + ' ' + messages.name.unnamed;
					}
				},
				address : {
					check: function(value, field) {
					...
					msg : function (field) {
						var fieldName = field.attr('data-fieldname');
						if (fieldName !== undefined && fieldName !== '') {
							return messages.prefix + ' ' + fieldName + ' ' + messages.address.named;
						}
						return messages.prefix + ' ' + messages.address.unnamed;
					}
				},
				alphaspace : {
					check: function(value, field) {
					...
					msg : function (field) {
						var fieldName = field.attr('data-fieldname');
						if (fieldName !== undefined && fieldName !== '') {
							return messages.prefix + ' ' + fieldName + ' ' + messages.alphaspace.named;
						}
						return messages.prefix + ' ' + messages.alphaspace.unnamed;
					}
				},
				alphanumericspacecomma : {
					check: function(value, field) {
						if (value) {
							// allow alpha, numeric, comma, space
							return testPattern (value, "^[0-9a-zA-Z, ]+$");
						}
						return true;
					},
					msg : function (field) {
						var fieldName = field.attr('data-fieldname');
						if (fieldName !== undefined && fieldName !== '') {
							return messages.prefix + ' ' + fieldName + ' ' + messages.alphanumericspacecomma.named;
						}
						return messages.prefix + ' ' + messages.alphanumericspacecomma.unnamed;
					}
				}
			};
	...
	Validate.prototype = {
		...
		scrollToError : function (selector) {
			...
			this.$element.find('input.error, select.error, textarea.error, .alert-box, .radio-group.error').get(0).focus();
	...
	$.fn.validate.defaults = {
		template : '<div class="error field-error-text validation-message-for-{{fieldId}}" id="error-msg-{{fieldId}}" aria-live="polite" tabindex="-1">' +
			'<span class="icon icon-error" aria-hidden="true"></span>{{errorMessage}}' +
			'</div>',
		live_validate: false,
		blur_validate: true,
