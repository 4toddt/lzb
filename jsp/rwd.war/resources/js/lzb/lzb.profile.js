...
(function (global, $, namespace) {
	...
	var CONSTANTS = global[namespace].constants,
			...
			persistPages = [
				...
				'/rwd/checkout/login.jsp',			// checkout login
				'/rwd/checkout/cart.jsp'				// cart page
			],
			profileController = {};
	...
	function updateCartQuantity(data) {
		if (data.cartCount > 0) {
			$(".masthead-mobile-link-cart .cart-count").text('(' + data.cartCount + ')');
			if (loggingDebug) {
			...
			$(".masthead-mobile-link-cart .cart-count").text('');
		}
	}
