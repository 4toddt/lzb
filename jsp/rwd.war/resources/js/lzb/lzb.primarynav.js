	...
	Primarynav.prototype = {
		...
		init: function init (type, element, options) {
			...
			if (!self.options.is_primary_hover) {
				...
				this.$element
						.off('primarynav')
						.on('mouseenter.' + namespace + '.primarynav, focus.' + namespace + '.primarynav', '.primary-nav-button', function (e) {
							if (!isMenuToggle(this)) {
								return;
							}
							var $menu;
		...
		close : function ($menu){
			if ($menu) {
				...
				$menu.find('.primary-nav-button a').attr('aria-expanded', false);
			} else {
				this.$element.find('.primary-nav-item').removeClass("active").find('.primary-nav-menu').hide();
				this.$element.find('.primary-nav-button a').attr('aria-expanded', false);
			}
		},
		open : function ($menu){
			var $dropdown = $menu.find('.primary-nav-menu'),
			...
			$menu.find('.primary-nav-button a').attr('aria-expanded', true);
		}
	};
...
}(this, window.jQuery, window.Modernizr, "LZB"));
...
(function (global, $, Modernizr, namespace) {
	...
	Flyoutnav.prototype = {
		...
		open: function(flyout){
			// checks for legacy utility nav from desktop header and adds simple support for flyout.
			if (this.$element.hasClass('legacy-utility-nav')) {
				flyout.show().parent().addClass('active');
			}
			// standard flyout support
			else {
				...
				flyout.attr('style', '').css({ top: 0, left: left, height: menuheight})
